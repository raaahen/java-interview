...
Метод `main()` — это точка входа в standalone Java-приложение (например, консольную программу). JVM (Java Virtual Machine) начинает выполнение кода именно с него. В одном проекте может быть несколько методов `main()` в разных классах, что полезно для тестирования или модульных запусков. Однако в одном классе не может быть двух методов с одинаковой сигнатурой `public static void main(String[] args)` — это вызовет ошибку компиляции (duplicate method).

Если метод `main()` отсутствует в указанном классе, компиляция пройдёт успешно (классы без `main()` — это просто обычные классы), но при запуске JVM выдаст ошибку:  
"Error: Main method not found in class ClassName, please define the main method as:  
public static void main(String[] args)".

### Минимальный пример метода `main()`
```java
public static void main(String[] args)
{
    // Здесь размещается код, который выполнится при запуске
}
```

### Что происходит, если в программе несколько методов `main()`?
- **Возможность существования**: Несколько `main()` могут быть в разных классах проекта. В одном классе возможны только перегруженные версии (с другой сигнатурой), но JVM будет искать и запускать только стандартную: `public static void main(String[] args)`.
- **Как определяется точка старта?**  
  JVM не выбирает автоматически — ты (или среда) указываешь класс для запуска:  
  - В командной строке: `java ClassName` (ищет `main()` в `ClassName`).  
  - В IDE: В настройках Run Configuration выбираешь класс.  
  - В JAR-файле: В манифесте (MANIFEST.MF) указывается `Main-Class: com.example.MyApp`.  
  Если класс не указан явно, приложение не запустится.  
- **Пример с несколькими `main()`**:  
  ...
  public class App1
  {
      public static void main(String[] args)
      {
          System.out.println("Запущен App1");
      }
  }
  ```  
  ```java
  ...
  public class App2
  {
      public static void main(String[] args)
      {
          System.out.println("Запущен App2");
      }
  }
  ```  
  Запуск `java App1` выполнит первый, `java App2` — второй.
...
Статический метод **может быть перегружен** так же, как и нестатический. Перегрузка (overloading) возможна, если два метода имеют одинаковое имя, но различаются по количеству, типам или порядку параметров. Возвращаемый тип не влияет на перегрузку.
...
Статический метод **нельзя переопределить** (override) в классическом смысле, как это работает с нестатическими методами в полиморфизме. Переопределение подразумевает позднее связывание (late binding), когда выбор метода зависит от типа объекта во время выполнения. Для статических методов используется раннее связывание (early binding), и выбор метода зависит от типа переменной, а не объекта.

Если в дочернем классе создать статический метод с той же сигнатурой, что и в родительском, это называется **скрытием (hiding)**, а не переопределением. Синтаксически это допустимо, но поведение отличается от полиморфизма.
...
{
    public static void show() // Скрытие, а не переопределение
    {
...
public class Main
{
    ...
    {
        Parent p = new Child();
        p.show(); // Выведет "Parent static method" (по типу переменной)
        Child.show(); // Выведет "Child static method" (через класс)
    }
}
```

### Рекомендации
К статическим полям и методам рекомендуется обращаться через имя класса (`ClassName.staticMethod()`), а не через объект (`obj.staticMethod()`). Хотя вызов через объект синтаксически допустим, это считается плохим стилем, так как статические члены не зависят от экземпляра класса и могут ввести в заблуждение.
...
**Локальный класс** (Local Inner Class) — это класс, объявленный внутри блока кода (например, в методе, конструкторе или блоке инициализации), где разрешено объявление переменных. Локальные классы являются подвидом внутренних классов, имеют имена (в отличие от анонимных классов) и могут использоваться многократно в пределах своего блока. Они связаны с экземпляром внешнего класса в нестатическом контексте.

**Особенности локальных классов**:
- **Область видимости**: Видны только внутри блока, в котором объявлены (например, метод или блок `{}`).  
- **Модификаторы доступа**: Не могут быть объявлены с модификаторами `private`, `public`, `protected` или `static`.  
- **Статические члены**: Не могут содержать статические методы, блоки инициализации или классы, но допускают `static final` поля (константы), инициализированные константным выражением.
- **Доступ к членам внешнего класса**: В нестатическом контексте имеют доступ ко всем полям и методам внешнего класса (включая `private`, статические и нестатические); в статическом контексте (например, в `static` методе) — только к статическим членам.  
- **Доступ к локальным переменным**: Могут обращаться к локальным переменным и параметрам метода, если они `final` или **эффективно финальные** (не изменяются после инициализации). Это связано с тем, что их значения копируются для использования в классе.  
- **Связь с внешним экземпляром**: В нестатическом контексте хранят неявную ссылку на внешний экземпляр (доступ через `Outer.this`); в статическом контексте такой ссылки нет.
- **Компиляция**: При компиляции получают имя вида `Outer$1LocalClass.class` (цифра зависит от порядка объявления).
...
public class Outer
{
    ...
    void instanceMethod()
    {
        final int localVar = 30; // final переменная
        int effectivelyFinalVar = 40; // Эффективно финальная (не изменяется)
    ...
    static void staticMethod()
    {
        ...
        // Локальный класс в статическом контексте
        class LocalClass
        {
            void accessMembers()
            {
                System.out.println(staticField); // OK
                System.out.println(localVar); // OK
                // System.out.println(nonStaticField); // Ошибка
            }
        }
...
**Совет**:
Используй локальные классы для разовой логики внутри метода, когда нужен доступ к локальным переменным или членам внешнего класса. Убедись, что локальные переменные `final` или эффективно финальные. Для многократного использования или сложной структуры лучше подойдут внутренние или статические вложенные классы.

[к оглавлению](#java-core)

## Что такое _«анонимные классы»_? Где они применяются?

**Анонимный класс** (Anonymous Inner Class) — это безымянный локальный класс, объявляемый и создаваемый в виде выражения в любом месте программы, где разрешены выражения (например, в методе, присваивании или аргументе). Он одновременно определяет класс и создаёт его экземпляр с помощью конструкции `new SuperClass() { ... }` или `new Interface() { ... }`. Анонимные классы наследуют существующий класс или реализуют интерфейс и используются для одноразовой реализации функциональности. В зависимости от контекста (статический или нестатический), они ведут себя как статические или внутренние вложенные классы.

**Особенности анонимных классов**:
- **Объявление и создание**: Объявляются и инстанцируются одновременно через `new`.  
- **Область видимости**: Видны только в месте объявления, так как не имеют имени и не могут быть переиспользованы.  
- **Ограничения по членам**:  
  - Не могут объявлять новые методы (кроме переопределения методов суперкласса или интерфейса), так как нет именованного типа для их вызова.  
  - Не могут содержать статические методы, блоки или классы, но допускают `static final` поля (константы).  
- **Доступ к членам внешнего класса**:  
  - В нестатическом контексте имеют доступ ко всем полям и методам внешнего класса (включая `private`) через `Outer.this`.  
  - В статическом контексте — только к статическим членам внешнего класса.  
- **Доступ к локальным переменным**: Могут обращаться к локальным переменным и параметрам метода, если они `final` или **эффективно финальные** (не изменяются после инициализации, с Java 8).  
- **Контекст**: В нестатическом контексте хранят ссылку на экземпляр внешнего класса; в статическом — нет.  
- **Компиляция**: Получают имя вида `Outer$1.class` (цифра зависит от порядка объявления).  

**Где применяются**:
1. **Функциональные объекты (Function Objects)**: Реализация интерфейсов, таких как `Comparator` или `ActionListener`, для одноразовой логики.
2. **Процессные объекты (Process Objects)**: Реализация `Runnable` или `Thread` для задач в потоках.  
3. **Инициализация сложных статических полей**: Создание `static final` полей, представляющих перечисления с подклассами.  
4. **Одноразовые реализации**: Передача реализаций интерфейсов или подклассов в методы без создания именованного класса.  

**Дополнительно**:
- С Java 8 анонимные классы для функциональных интерфейсов (с одним абстрактным методом) часто заменяются лямбда-выражениями, так как они короче.  
- Могут вызывать утечки памяти, если их объекты живут дольше внешнего класса (например, в слушателях событий).  
- Могут реализовывать только один интерфейс или наследовать один класс за раз.  

**Примеры**:
1. **Реализация `Comparator` (Function Object)**:
```java
...
public class Outer
{
    ...
    public static void main(String[] args)
    {
        Outer outer = new Outer();
        int[] array = {3, 1, 4};
        outer.sortArray(array);
        System.out.println(Arrays.toString(array)); // [4, 3, 1]
    }
}
```

2. **Реализация `Runnable` (Process Object)**:
```java
public class Outer
{
    private int nonStaticField = 10;
    ...
    void startThread()
    {
        Thread thread = new Thread(new Runnable()
        {
            ...
            public void run()
            {
                System.out.println("Non-static field: " + nonStaticField); // OK
                System.out.println("From anonymous class in thread");
            }
        });
        thread.start();
    }
    ...
    public static void main(String[] args)
    {
        new Outer().startThread();
    }
}
```

3. **Инициализация статического поля**:
```java
public class Outer
{
    private static int staticField = 20;
    ...
    static class MyEnum
    {
        static final MyEnum VALUE1 = new MyEnum()
        {
            @Override
            void doSomething()
            {
                System.out.println("VALUE1: " + staticField);
            }
        };
    ...
    public static void main(String[] args)
    {
        MyEnum.VALUE1.doSomething(); // VALUE1: 20
    }
}
```
...
public class Outer
{
    ...
    public static void main(String[] args)
    {
        new Outer().methodWithLocalVar(); // Local var: 30, Effectively final var: 40
    }
}
```

**Совет**:
Используй анонимные классы для одноразовой логики, когда не нужен именованный класс. Для функциональных интерфейсов предпочитай лямбда-выражения: `Arrays.sort(array, (a, b) -> b - a)`. Убедись, что локальные переменные `final` или эффективно финальные. Избегай длительного хранения объектов анонимных классов в нестатическом контексте, чтобы предотвратить утечки памяти.
...
+ Создание нового массива с заранее неизвестным типом.
...
**Ответ**:  
Хэш-код `31 * x + y` предпочтительнее `x + y`, так как обеспечивает лучшее распределение значений и снижает количество коллизий в хэш-таблицах (`HashMap`, `HashSet`), что улучшает производительность. Вот подробное объяснение:

- **Проблемы с `x + y`**:  
  - **Высокая вероятность коллизий**: Разные точки могут иметь одинаковый хэш-код. Например:  
    - `Point(1, 2)`: хэш = 1 + 2 = 3  
    - `Point(2, 1)`: хэш = 2 + 1 = 3  
    - `Point(0, 3)`: хэш = 0 + 3 = 3  
    Это приводит к помещению разных объектов в одну "корзину" хэш-таблицы, замедляя операции поиска, вставки и удаления (с O(1) до O(n) в худшем случае).  
  - **Игнорирование порядка полей**: Сумма не учитывает последовательность полей, что увеличивает коллизии.  
  - **Слабое распределение**: Хэш-коды группируются в узком диапазоне (например, 0–2000 для малых x, y), не используя весь диапазон `int` (-2^31 до 2^31-1).  

- **Преимущества `31 * x + y`**:  
  - **Учёт порядка полей**: Множитель 31 делает хэш чувствительным к последовательности полей:  
    - `Point(1, 2)`: хэш = 31 * 1 + 2 = 33  
    - `Point(2, 1)`: хэш = 31 * 2 + 1 = 63  
    Разные точки — разные хэши, что снижает коллизии.  
  - **Равномерное распределение**: Простое число 31 "разбрасывает" хэш-коды по диапазону `int`, минимизируя совпадения. 31 выбрано, так как:  
    - Простое (избегает паттернов в модульной арифметике).  
    - Близко к 32 (оптимизировано: `31 * x = (x << 5) - x`).  
    - Эмпирически даёт хорошее распределение (по Дж. Блоху, "Effective Java").  
  - **Обобщение для сложных классов**: Для классов с несколькими полями формула: `result = 31 * result + fieldHash`, где `fieldHash` — хэш очередного поля.  

- **Соответствие контракту**: Хэш-код должен согласовываться с `equals()` (одинаковые объекты — одинаковый хэш, разные — желательно разные). `31 * x + y` лучше выполняет это для неравных объектов.  
...
public class Point
{
    ...
    public int hashCode()
    {
        return 31 * x + y; // Или Objects.hash(x, y) в Java 7+
    }
    ...
    public static void main(String[] args)
    {
        ...
        System.out.println(p2.hashCode()); // 63
    }
}
```
...
Используй `31 * x + y` или `Objects.hash(x, y)` для простых классов с примитивами. Для reference-типов (String, Object) вычисляй хэш поля: `(field == null ? 0 : field.hashCode())`.
Тестируй хэш-функцию на больших наборах данных, чтобы убедиться в низком числе коллизий.
Для больших проектов измеряй производительность с помощью JMH, особенно если хэш-таблицы содержат миллионы объектов.
