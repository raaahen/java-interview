...
Хэш-код `31 * x + y` предпочтительнее `x + y`, так как обеспечивает лучшее распределение значений и снижает количество коллизий в хэш-таблицах (`HashMap`, `HashSet`), что улучшает производительность.
